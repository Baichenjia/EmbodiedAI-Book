...
- 面向多款 Unitree 机器人的统一训练与部署流程。
- 基于 MuJoCo + Warp 的高吞吐并行仿真，适合大规模 RL 训练。
- 同时覆盖速度跟踪与动作模仿两类核心任务。
- 提供仿真优先的部署链路，降低实机调试风险。
...
三组实验均使用 **10240 个并行环境**，运行设备为 `cuda:0`。

| 任务 | 日志目录 | 日志内迭代进度 | 平均奖励变化 | 吞吐变化（steps/s） | 状态 |
|------|----------|----------------|--------------|---------------------|------|
| G1 速度跟踪（`Unitree-G1-Flat`） | `logs/rsl_rl/g1_velocity/2026-05-05_03-27-52` | `0/10001 -> 2034/10001` | `-1.65 -> 41.62` | `11696 -> 12747` | 训练持续推进，奖励显著提升 |
| G1 动作跟踪（`Unitree-G1-Tracking`） | `logs/rsl_rl/g1_tracking/2026-05-06_02-13-49` | `0/30001 -> 1450/30001` | `-1.64 -> 21.76` | `11053 -> 12734` | 因磁盘空间不足中断（`OSError: [Errno 28] No space left on device`） |
| G1 动作跟踪无状态估计（`Unitree-G1-Tracking-No-State-Estimation`） | `logs/rsl_rl/g1_tracking/2026-05-05_05-06-31` | `0/30001 -> 3910/30001` | `-2.01 -> 26.28` | `11007 -> 13219` | 因磁盘空间不足中断（`OSError: [Errno 28] No space left on device`） |

### 结果说明

- 速度跟踪任务在当前配置下表现出稳定的奖励上升趋势。
- 两组动作模仿任务在中断前也呈现明显收益提升。
- 日志中存在 `CUDA Graphs disabled: driver 12.2 < 12.4` 提示；当前可训练，但升级驱动可能带来更好的稳定性与性能。
