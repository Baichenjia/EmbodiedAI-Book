...
- Unified training and deployment workflow for multiple Unitree robots.
- High-throughput MuJoCo + Warp simulation suitable for large-scale RL.
- Covers both velocity tracking and motion imitation tasks in one codebase.
- Includes sim-first deployment path before transferring to real robots.
...
## 📊 Program Run Results (from `logs/`)

The following summary is extracted from:

- `logs/log_G1_Flat.txt`
- `logs/log_G1-Tracking.txt`
- `logs/log_G1-Tracking-No-State-Estimation.txt`

All three runs were launched with **10240 environments** on `cuda:0`.

| Task | Log directory | Iteration progress in log | Mean reward trend | Throughput trend (steps/s) | Status |
|------|---------------|---------------------------|-------------------|----------------------------|--------|
| G1 Velocity (`Unitree-G1-Flat`) | `logs/rsl_rl/g1_velocity/2026-05-05_03-27-52` | `0/10001 -> 2034/10001` | `-1.65 -> 41.62` | `11696 -> 12747` | Running progressed and reward improved significantly |
| G1 Tracking (`Unitree-G1-Tracking`) | `logs/rsl_rl/g1_tracking/2026-05-06_02-13-49` | `0/30001 -> 1450/30001` | `-1.64 -> 21.76` | `11053 -> 12734` | Interrupted by disk-full error (`OSError: [Errno 28] No space left on device`) |
| G1 Tracking No State Estimation (`Unitree-G1-Tracking-No-State-Estimation`) | `logs/rsl_rl/g1_tracking/2026-05-05_05-06-31` | `0/30001 -> 3910/30001` | `-2.01 -> 26.28` | `11007 -> 13219` | Interrupted by disk-full error (`OSError: [Errno 28] No space left on device`) |

### Result interpretation

- The velocity-tracking setting shows stable reward growth and sustained training speed.
- Both motion-imitation settings also show reward improvement before interruption.
- Main blocker in the failed runs is **storage exhaustion**, not obvious algorithm instability.
- Logs also show `CUDA Graphs disabled: driver 12.2 < 12.4`; training still proceeds, but newer driver versions may provide better stability/performance.
